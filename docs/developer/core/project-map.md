...
- **Key folders**:
  ...
  - `src/pages/ssh`: SSH terminal controllers and pure runtime helpers. `use-ssh-core.ts` coordinates pane routing; primary/secondary hooks own independent session resources; `ssh-pane-state.ts` reduces all pane-scoped transport/helper messages; `terminal-presentation-state.ts` owns the transport-independent pane presentation domain, `terminal-presentation-integration.ts` passively binds that domain to complete xterm parser/title/Bell callbacks without coupling it to Remote Enhancements, and `terminal-presentation-tab-state.ts` applies active-pane and background-attention aggregation rules; `ssh-command-markers.ts` owns pending/confirmed xterm marker lifecycles and pane-local command timeline models; `TerminalCommandTimeline.tsx` renders the trusted right-side command rail.

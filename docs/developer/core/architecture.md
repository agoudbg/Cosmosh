...
- Terminal Presentation Integration is a separate pane-scoped renderer domain. PTY output reaches `terminal.write(...)` unchanged, and the xterm parser is the only control-sequence parser for OSC 0/2 application titles, OSC 9;4 progress, and standalone BEL events. The resulting memory-only state is reset on reconnect and removed with its pane, then projected through pure tab and window aggregators into the active-pane title, prioritized progress, retained Bell/error attention, and one window activity snapshot. `App` applies user presentation policy only at this projection boundary, supplies the ephemeral tab projection to chrome, and sends only the window snapshot through preload; it never disables passive parsing or writes application titles into stored tab identity. This domain neither depends on nor enables Shell Integration, Remote Bootstrap, or OSC 777 Remote Enhancements.
- Window activity crosses one sender-bound `app:set-terminal-window-activity` IPC channel. Main resolves the owning `BrowserWindow` from `event.sender`, validates the shared IPC-only payload again, maps `error > warning > indeterminate > normal > none` to Electron taskbar modes, clears absent progress with `setProgressBar(-1)`, and applies audible Bell and unfocused-window Flash effects according to the renderer-provided policy. Bell identity is the current renderer-document epoch plus a renderer-window monotonic sequence, independent from acknowledgement state, so rerenders, settings changes, progress completion, renderer reload, and revealing an older event after tab closure cannot replay attention. Main samples its own monotonic clock for independent sound and Flash throttles; renderer data never controls the throttle interval.
...
- The sandboxed preload script must not import workspace packages at runtime. It may use shared API contract types at compile time, while fixed bridge methods forward only to their allow-listed channels and Main performs zero-trust payload validation with the shared runtime contract. Any validation that must execute inside preload must stay local or be bundled so Electron does not need to resolve project modules before the bridge loads.

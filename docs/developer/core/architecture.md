...
- Six tools are exposed: `list_servers` (credential-free and optionally approval-gated), `open_connection` (always prompts the user), `attach_terminal` (the user selects an existing eligible SSH pane), `list_connections`, `run_command` (policy-gated and bounded), and `close_connection`. The shared connection cap is 8 and the idle timeout is 10 minutes.
- `open_connection` defaults to `terminal`: approval creates and focuses a normal renderer SSH tab, then a 60-second backend launch broker binds its primary session. `background` explicitly retains isolated `ssh2.exec` with separate stdout/stderr/exit metadata. `attached` never enumerates panes to the Agent; the renderer selects one and submits only its private session id through the internal management API.
- Visible terminal automation is fail-closed on the complete trusted Remote Enhancements contract (`command-start`, `command-end`, `prompt-ready`, and `line-state`). One Agent may attach to a session and run one command at a time. Output is captured between matching trusted command ids; cancellation/timeout stops the MCP wait but does not send `Ctrl+C`.
- User input remains enabled while an Agent command runs and sets `userIntervened`; the Agent status bar exposes explicit Stop (`Ctrl+C`) and Detach controls. Explicit Agent/UI close owns an Agent-created tab, while client disconnect, token revoke, disable, and idle cleanup detach the Agent and preserve visible tabs.
- `list_servers` raises an in-app authorization dialog when the global `mcpListServersRequiresApproval` setting is enabled; the default is disabled for backward compatibility. The service does not query or return server metadata before approval. Opening a connection **always** raises an authorization dialog. Command execution follows the effective policy — `server.mcpCommandPolicy` when not `default`, otherwise the global `mcpCommandPolicy` — resolving to `off` (rejected), `ask` (per-command prompt), or `allowWithinConnection` (first command prompts, remainder may be allowed for that connection). Unanswered authorizations time out as denials after 120 seconds; if no renderer window is open they can only time out.

...
- Six tools are exposed: `list_servers` (credential-free), `open_connection` (always prompts the user), `attach_terminal` (the user selects an existing eligible SSH pane), `list_connections`, `run_command` (policy-gated and bounded), and `close_connection`. The shared connection cap is 8 and the idle timeout is 10 minutes.

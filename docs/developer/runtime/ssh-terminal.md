...
- `terminal.onBell(...)` is the only Bell attention source. A BEL used to terminate OSC 0/2 or OSC 9;4 is consumed by xterm as a terminator and does not produce a standalone Bell event. OSC 9;4 state `0` clears progress only and never synthesizes Bell attention. Every standalone Bell receives a renderer-window monotonic sequence before it enters pane state, which establishes a clock-independent total order across panes and tabs within the current renderer epoch.
- Presentation state belongs to each pane independently. A connection retry clears only that pane's stale title/progress/Bell state. An unexpected transport close or error runs after pending xterm writes, clears the ended session's application title and progress, and retains any Bell attention that the user has not acknowledged. Terminal disposal unregisters all parser listeners, and pane removal deletes the pane state.
...
- Focusing a pane acknowledges Bell attention for that pane only when the renderer document is visible and its window owns focus. A retained DOM active element or programmatic terminal focus while the window is in the background does not acknowledge attention. A standalone Bell received while its active pane is focus-exposed is acknowledged immediately; switching tabs follows the same gate, and returning to the focused visible window acknowledges the active pane.
...
- The latest Bell event keeps `{ rendererEpoch, tabId, paneId, sequence }` independently from current Bell attention. A random epoch identifies one renderer document, and its renderer-window monotonic sequence totally orders Bell edges across panes and tabs. Pane/tab/window aggregation ignores metadata from an older epoch after reload or Fast Refresh; Main resets its sequence high-water mark only when the epoch changes and records every accepted edge before applying effect policy, so disabled, throttled, or later-revealed events cannot replay. Audible Bell and unfocused-window Flash use independent one-second windows sampled from Main's monotonic clock; renderer data cannot advance those throttles, and window focus stops an active Flash. Progress state `none` never enters this Bell path.
...
- `pnpm --filter @cosmosh/renderer test:ssh` covers fragmented OSC input, Kimi Code's valueless OSC 9;4 progress forms, malformed OSC 9;4 states, OSC-terminating BEL versus standalone BEL, renderer-epoch Bell ordering, pane/tab aggregation, acknowledgement, reconnect cleanup, and settings projection.
- `pnpm --filter @cosmosh/main test:terminal-presentation` covers window progress mapping, sender payload validation, renderer epoch/sequence replay protection, sound/Flash policy, and Main-monotonic independent throttling.

...
- `terminal.onBell(...)` is the only Bell attention source. A BEL used to terminate OSC 0/2 or OSC 9;4 is consumed by xterm as a terminator and does not produce a standalone Bell event. OSC 9;4 state `0` clears progress only and never synthesizes Bell attention. Every standalone Bell receives a renderer-window monotonic sequence before it enters pane state, so equal wall-clock timestamps across panes or tabs still have a total order.
...
- The latest Bell event keeps `{ tabId, paneId, sequence, receivedAt }` independently from current Bell attention. `sequence` is renderer-window monotonic and breaks equal-`receivedAt` ties before Main consumes each event at or above its receipt-time high-water mark. A disabled or throttled event therefore cannot replay after a setting change. Audible Bell and unfocused-window Flash use independent one-second throttle windows; window focus stops an active Flash. Progress state `none` never enters this Bell path.

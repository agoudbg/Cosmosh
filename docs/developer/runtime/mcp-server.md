...
- Listing configured servers can require explicit authorization through `mcpListServersRequiresApproval` (default `false`). When enabled, Cosmosh does not query or return server metadata before approval.
...
| Tool               | Input                                                                                                              | Behavior                                                                                                                                                                                                                                             |
| ------------------ | ------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `list_servers`     | `query?` (string, ≤200)                                                                                            | Optionally raises an authorization prompt according to `mcpListServersRequiresApproval`, then returns `{ servers, count }`; each entry has `serverId`, `name`, `host`, `port`, `username`, effective `commandPolicy`, `folder?`, `tags`, `note?`. Never returns credentials. Read-only. |
...
When server-list approval is enabled, `list_servers` can return `denied`, `timeout`, or `audit-unavailable`. These outcomes are returned before the SSH server table is queried, so no server metadata is included.
...
| `list-servers`                                       | Each `list_servers` outcome, including approval denial, timeout, or audit failure.                       |
...
- **Settings:** `mcpEnabled` (default `false`), `mcpListServersRequiresApproval` (default `false`), and `mcpCommandPolicy` (default `ask`) live in the settings registry under the `mcp` category (sections `mcpAccess` / `mcpPolicy`). Runtime status, pairing, client configuration, connections, and approvals are composed into that same Settings category; MCP has no standalone workbench page or tab. The server-list approval setting is global and has no per-server override.

...
- Queued archive work resolves the tab's latest session when the renderer-serialized task starts. After an archive operation is accepted, polling, cancellation, and conflict resolution continue using that operation's owning session even if passive reconnect changes the tab's current session.
...
- CodeMirror preview keyboard shortcuts, including `Ctrl`/`Cmd+S` save and `Ctrl`/`Cmd+F` find/replace, stay scoped to the editor. Its context menu uses the shared Cosmosh text-editing menu surface for undo, redo, find/replace, cut, copy, paste, and select-all. The find/replace panel uses the reusable renderer `SearchReplacePanel`; read-only previews keep find enabled and expose replacement controls in a readonly state. Because the preview pane is usually a narrow sidebar, the panel uses the CodeMirror adapter's `docked-bottom` placement: it pins to the editor's bottom edge as a full-width bar in the layout flow, so controls never clip against the pane width and preview content shifts up instead of being covered by a floating overlay; below 480px of pane width the panel collapses into a stacked layout (search input with close button, replace input with its actions, then a find-options row). SFTP page-level file-list shortcuts and global fallback context menus ignore events that originate from editor, text-input, or contenteditable targets.
...
- Main owns one per-run SFTP temp root created with `mkdtemp` under Electron's temp directory. Main validates the root with `lstat` and `realpath`, rejects symlink roots, uses private POSIX modes for the root, child temp directories, and staged files, then passes the canonical root to backend through `COSMOSH_SFTP_TEMP_ROOT`. Before creating each isolated upload/download directory, Main revalidates the root. If an OS or external temp cleaner removed it, Main recreates the same canonical path and retries once, preserving the path already held by Backend; concurrent recovery is idempotent. Existing non-directory, symlink, canonical-path-changing, or non-private replacements remain hard failures and are never removed or reused. Backend restart performs the same revalidation, and Backend refuses to start with a missing, symlinked, non-directory, or non-private root on POSIX platforms.

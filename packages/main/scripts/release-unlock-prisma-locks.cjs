 ...
 */
const script = [
  `$seedProtectedPids = @(${protectedPids.join(',')})`,
  '$processMap = @{}',
  'Get-CimInstance Win32_Process | ForEach-Object {',
  '  $processMap[[int]$_.ProcessId] = $_',
  '}',
  "$protectedPidSet = New-Object 'System.Collections.Generic.HashSet[int]'",
  '$seedProtectedPids | ForEach-Object {',
  '  [void]$protectedPidSet.Add([int]$_)',
  '}',
  'foreach ($seedPid in $seedProtectedPids) {',
  '  $cursor = [int]$seedPid',
  '  while ($processMap.ContainsKey($cursor)) {',
  '    $parentPid = [int]$processMap[$cursor].ParentProcessId',
  '    if ($parentPid -le 0 -or $protectedPidSet.Contains($parentPid)) {',
  '      break',
  '    }',
  '    [void]$protectedPidSet.Add($parentPid)',
  '    $cursor = $parentPid',
  '  }',
  '}',
  "$targets = Get-CimInstance Win32_Process | Where-Object {",
  "  ($_.Name -in @('node.exe','electron.exe','Cosmosh.exe')) -and",
  '  -not $protectedPidSet.Contains([int]$_.ProcessId) -and',
